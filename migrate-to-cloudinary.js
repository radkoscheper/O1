 ...
 */
function generatePublicId(relativePath, filename) {
...
async function updateDatabaseUrls() {
  ...
  try {
    ...
    const siteSettingsRecords = await db.select().from(siteSettings);
    for (const settings of siteSettingsRecords) {
      let updated = false;
      const updates = {};
      
      if (settings.backgroundImage && settings.backgroundImage.startsWith('/images/')) {
        const cloudinaryUrl = findCloudinaryUrl(settings.backgroundImage);
        if (cloudinaryUrl) {
          updates.backgroundImage = cloudinaryUrl;
          updated = true;
        }
      }
      
      if (settings.favicon && settings.favicon.startsWith('/images/')) {
        const cloudinaryUrl = findCloudinaryUrl(settings.favicon);
        if (cloudinaryUrl) {
          updates.favicon = cloudinaryUrl;
          updated = true;
        }
      }
      ...
      if (updated) {
        ...
        console.log(`Updated site settings:`, updates);
      }
    }
...
async function migrate() {
  ...
  if (migrationLog.uploaded.length > 0) {
    ...
    migrationLog.uploaded.slice(0, 5).forEach(item => {
      console.log(`${item.localPath} -> ${item.cloudinaryUrl}`);
    });
  }
}
